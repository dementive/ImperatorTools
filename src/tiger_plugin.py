...
class ImperatorTigerEventListener(sublime_plugin.EventListener):
    ...
    def on_load_async(self, view):
        if not self.settings.get("ImperatorTigerShowErrorsInline"):
            return
...
def get_tiger_objects():
    ...
    path = sublime.packages_path() + "/ImperatorTools/tiger.json"
...
class ShowTigerOutputCommand(sublime_plugin.WindowCommand):
    ...
    def annotation_callback(self, string):
        ...
        path = self.settings.get("ImperatorTigerModPath") + "\\" + string[0]

        if not os.path.exists(path):
            path = self.imperator_files_path + "\\" + string[0]
...
class ExecuteTigerCommand(sublime_plugin.WindowCommand):
    ...
    def on_finished(self, proc):
        ...
        if json_start_index != -1:
            ...
            output_file = sublime.packages_path() + "/ImperatorTools/tiger.json"
    ...
    def run(self):
        ...
        if not os.path.exists(tiger_exe_path):
            tiger_exe_path = (
                ...
                + "/ImperatorTools/ImperatorTiger/imperator-tiger.exe"
            )
